...
public class GroundControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _wheelArray = GetComponentsInChildren<Wheel>();
    }
    ...
    private void FixedUpdate()
    {
        ...
        var forwardAcceleration = CalcForwardForce(GameManager.MobileInput.throttleInput);
    ...
    private void SetDriftFriction()
    {
        ...
        var currentDriftDrag = GameManager.MobileInput.isDrift ? wheelSideFrictionDrift : wheelSideFriction;
    ...
    private float CalcForwardForce(float throttleInput)
    {
        ...
        if (GameManager.MobileInput.isBoost)
    ...
    private float CalculateSteerAngle()
    {
        ...
        return GameManager.MobileInput.steerInput *  curvature * turnRadiusCoefficient;
    }
    ...
    static float GetForwardAcceleration(float speed)
    {
        ...
        if (speed > (1410 / 100))
            ...
            throttle = PocketUtils.Scale(14, 14.1f, 1.6f, 0, speed);
        else if (speed <= (1400 / 100))
            throttle = PocketUtils.Scale(0, 14, 16, 1.6f, speed);
    ...
    static float GetTurnRadius(float speed)
    {
        ...
        var curvature = PocketUtils.Scale(0, 5, 0.0069f, 0.00398f, forwardSpeed);

        if (forwardSpeed >= 500 / 100)
            curvature = PocketUtils.Scale(5, 10, 0.00398f, 0.00235f, forwardSpeed);

        if (forwardSpeed >= 1000 / 100)
            curvature = PocketUtils.Scale(10, 15, 0.00235f, 0.001375f, forwardSpeed);

        if (forwardSpeed >= 1500 / 100)
            curvature = PocketUtils.Scale(15, 17.5f, 0.001375f, 0.0011f, forwardSpeed);

        if (forwardSpeed >= 1750 / 100)
            curvature = PocketUtils.Scale(17.5f, 23, 0.0011f, 0.00088f, forwardSpeed);
